...
function printRunSpecification(value: unknown): string | null {
  if (!value || typeof value !== 'object' || Array.isArray(value)) return null;
  const printRun = (value as Record<string, unknown>).printRun;
  if (!printRun || typeof printRun !== 'object' || Array.isArray(printRun)) return null;
  const record = printRun as Record<string, unknown>;
  const number = (key: string) => typeof record[key] === 'number' && Number.isSafeInteger(record[key])
    ? record[key] as number
    : null;
  const runLabel = typeof record.runOptionLabel === 'string' ? record.runOptionLabel : null;
  const unitsPerRun = number('unitsPerRun');
  const lotCount = number('lotCount');
  const totalUnits = number('totalUnits');
  const leadDays = number('productionLeadTimeBusinessDays');
  const bleedRequired = record.artworkBleedAcknowledgementRequired === true;
  const bleedAcknowledged = record.artworkBleedAcknowledged === true;
  const parts = [
    runLabel && unitsPerRun !== null ? `Tiragem: ${runLabel} (${unitsPerRun.toLocaleString('pt-BR')} un./lote)` : null,
    lotCount !== null && totalUnits !== null ? `${lotCount} lote(s) = ${totalUnits.toLocaleString('pt-BR')} un.` : null,
    leadDays !== null ? `prazo: ${leadDays} dia(s) útil(eis)` : null,
    bleedRequired ? `sangria/margem segura: ${bleedAcknowledged ? 'ciência registrada' : 'pendente'}` : null,
    record.artworkFileRequired === true ? 'arte: pré-impressão obrigatória' : null,
  ].filter((part): part is string => Boolean(part));
  return parts.length > 0 ? parts.join(' · ') : null;
}
