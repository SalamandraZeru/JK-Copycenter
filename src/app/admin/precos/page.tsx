...
import { Calculator, Check, Loader2, Percent, Plus, Trash2, X } from 'lucide-react';
...
export default function PrecosPage() {
  const [activeTab, setActiveTab] = useState<'regras' | 'descontos'>('regras');
  ...
  const handleDeactivateRule = async (ruleId: string) => {
    if (!confirm('Deseja inativar esta regra de precificação?')) return;
    try {
      const response = await fetch(`/api/admin/precos/regras?id=${ruleId}`, { method: 'DELETE' });
      if (!response.ok) throw new Error();
      await mutateRules();
    } catch { alert('Erro ao inativar regra'); }
  };
