...
interface PricingService { id: string; name: string; pricing_profile?: string; }
...
export default function PrecosPage() {
  ...
  const selectedService = services.find((service) => service.id === ruleForm.service_id);
  const rulePriceLabel = selectedService?.pricing_profile === 'per_print_run'
    ? 'Preço por tiragem (R$) *'
    : selectedService?.pricing_profile === 'booklet_imposition'
      ? 'Preço do miolo por página de produção (R$) *'
    : selectedService?.pricing_profile === 'per_square_meter'
      ? 'Preço por m² (R$) *'
      : 'Preço por página (R$) *';
      ...
      {activeTab === 'regras' && <div className="space-y-6">
        ...
        {showRuleModal && <div className="bg-white rounded-3xl border-2 border-blue-500 shadow-xl p-6 sm:p-8 space-y-6 animate-in fade-in duration-200">
          ...
          <form onSubmit={handleCreateRule} className="space-y-5">
            ...
            <div className="grid grid-cols-1 sm:grid-cols-2 gap-4"><div><label className="block text-xs font-semibold uppercase tracking-wider text-slate-800 mb-1.5">{rulePriceLabel}</label><input type="number" min="0" step="0.01" value={ruleForm.price_per_page} onChange={(event) => setRuleForm((current) => ({ ...current, price_per_page: Number(event.target.value) }))} className="w-full px-4 py-2.5 bg-white border border-slate-300 rounded-xl text-slate-900 font-medium shadow-sm focus:border-blue-600 focus:ring-2 focus:ring-blue-600/20 outline-none transition" /></div><div><label className="block text-xs font-semibold uppercase tracking-wider text-slate-800 mb-1.5">Sem regra correspondente</label><select value={ruleForm.fallback_behavior} onChange={(event) => setRuleForm((current) => ({ ...current, fallback_behavior: event.target.value as RuleForm['fallback_behavior'] }))} disabled={selectedService?.pricing_profile === 'per_print_run' || selectedService?.pricing_profile === 'booklet_imposition'} className="w-full px-4 py-2.5 bg-white border border-slate-300 rounded-xl text-slate-900 font-medium shadow-sm focus:border-blue-600 focus:ring-2 focus:ring-blue-600/20 outline-none cursor-pointer transition disabled:bg-slate-100 disabled:text-slate-500"><option value="base_price">Usar preço base do serviço</option><option value="block">Bloquear pedido (não calculável)</option></select>{selectedService?.pricing_profile === 'per_print_run' && <p className="mt-1 text-xs font-medium text-slate-500">Tiragens exigem regra específica para cada combinação publicada.</p>}{selectedService?.pricing_profile === 'booklet_imposition' && <p className="mt-1 text-xs font-medium text-slate-500">Livreto exige regra específica para o miolo; capa, acabamento e mínimo do lote são exibidos separadamente.</p>}</div></div>
